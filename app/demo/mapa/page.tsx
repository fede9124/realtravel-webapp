...
import { LUGARES, COMERCIOS, CATEGORIAS } from '@/lib/data'
import type { MapPlace, MapComercio } from '@/app/mapa/page'
...
const PV_PLACES: MapPlace[] = LUGARES
  .filter((l): l is typeof l & { lat: number; lng: number } =>
    l.destinoId === 'puerto-varas' && l.lat !== undefined && l.lng !== undefined
  )
  .map(l => ({
    id: l.id,
    category: l.category,
...
const PV_COMERCIOS: MapComercio[] = COMERCIOS
  .filter((c): c is typeof c & { lat: number; lng: number } =>
    c.destinoId === 'puerto-varas' && c.lat !== undefined && c.lng !== undefined
  )
  .map(c => ({
    id: c.id,
    title: c.title,
...
export default function DemoMapaPage() {
  ...
  const [flyToTarget, setFlyToTarget] = useState<{ lat: number; lng: number; zoom?: number } | null>({ lat: -41.319, lng: -72.985, zoom: 11 })
  ...
  const mapPlaces = useMemo(() =>
    activeCategoria === null
      ? PV_PLACES
      : PV_PLACES.filter(p => p.categoria === activeCategoria)
  , [activeCategoria])

  const searchResults = useMemo(() => {
    const q = norm(searchQuery.trim())
    if (!q) return [] as ListItem[]
    const places: ListItem[] = PV_PLACES
      .filter(p => norm(`${p.title} ${p.location}`).includes(q))
      .slice(0, 4)
      .map(p => ({ ...p, kind: 'lugar' as const }))
    const comercios: ListItem[] = PV_COMERCIOS
  ...
  const filtered = useMemo(() => {
    ...
    let comercios: ListItem[] = PV_COMERCIOS.map(c => ({
    ...
    <>
            ...
            >
              {searchResults.map(place => (
                ...
                >
                  ...
                  {place.location && <span className="text-xs flex-shrink-0" style={{ color: 'var(--color-text-muted)' }}>· {place.location}</span>}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="flex-1 overflow-y-auto">
        {filtered.map(item => {
          ...
          const detailHref = item.kind === 'comercio' ? `/demo/servicios/${item.id}` : `/demo/atractivos/${item.id}`
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>
                    {item.kind === 'comercio' ? `Servicios · ${item.category}` : `${item.category} · ${item.location}`}
                  </p>
                  {item.description && (
